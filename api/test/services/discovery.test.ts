...
import { discoveryQueue, characterStatic, corporationStatic, characterEphemeral, corporationEphemeral } from '../../src/db/schema.ts';
...
Deno.test('Discovery Integration', async (t) => {
  ...
  await t.step('processing the queue should fetch the corporation', async () => {
    const queued = await db.select().from(discoveryQueue)
      .where(and(eq(discoveryQueue.entityType, 'corporation'), eq(discoveryQueue.status, 'pending')))
      .limit(1);
    ...
    const corp = await db.select().from(corporationStatic).where(eq(corporationStatic.corporationId, targetCorpId));
