...
Deno.test('Entity Schema', async (t) => {
  ...
  await t.step('should insert and retrieve a corporation with all fields', async () => {
    ...
    await db.delete(schema.corporationStatic).where(
      eq(schema.corporationStatic.corporationId, corpId),
    );
    ...
    const [retrievedEphemeral] = await db.select().from(schema.corporationEphemeral).where(
      eq(schema.corporationEphemeral.corporationId, corpId),
    );
  ...
  await t.step('should insert and retrieve an alliance with all fields', async () => {
    ...
    const [retrievedStatic] = await db.select().from(schema.allianceStatic).where(
      eq(schema.allianceStatic.allianceId, allianceId),
    );
