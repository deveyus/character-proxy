...
/**
 * Gets a corporation by ID, using local cache or fetching from ESI.
 */
export async function getById(
  id: number,
): Promise<Result<db.CorporationEntity | null, Error>> {
  try {
...
export async function getByName(
  ...
  try {
    ...
    if (localResult.value) {
      return getById(localResult.value.corporationId);
    }
